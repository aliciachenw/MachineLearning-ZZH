...
if __name__ == "__main__":
    ...
    with open("data/hayes-roth.csv", mode='r') as data_file:
        dataSet = pd.read_csv(data_file, header=0)
    n = len(dataSet[dataSet.columns[-1]])
    ...
    for label in labelCount:
        ...
        if flag == 0:
            flag = 1
    acc_id3 = np.zeros([10, 3])
    acc_cart = np.zeros([10, 3])
    for t in range(10):
    ...
    df = pd.DataFrame(result, columns=["id3", "id3-pre", "id3-post", "cart", "cart-pre", "cart-post"])
